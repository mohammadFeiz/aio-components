...
import { I_tasvireMadarek } from "../types";

const TasvireMadarek: FC<{
    data:I_tasvireMadarek,
    onChange:(v:I_tasvireMadarek)=>void
}> = (props) => {
    const getImageLayout = (label: string, field: any): I_formNode<I_tasvireMadarek> => {
        return {
    ...
    const form = useForm<I_tasvireMadarek>({
        initData: props.data, fa: true, inlineLabel: true,
        onChange: (tasvireMadarek) => props.onChange(tasvireMadarek),
        getLayout: (context) => {
