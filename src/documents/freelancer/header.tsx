...
const Header: FC = () => {
    const { filter, changeFilter,options } = useContext(CTX)
                        ...
                        h: [
                            {
                                size: 160,
                                input: { 
                                    type: 'select', label: 'هاب', field: 'hubId',
                            ...
                            },
                            {
                                size: 120,
                                input: {
                                    type: 'checkbox', label: '', field: 'faal', text: 'فعال', className:'brd-none-',
...
const ResetButton: FC = () => {
    const { resetFilter } = useContext(CTX)
    return (
        <button type='button' className="brd-none- bg-none- flex-row- align-v-" style={{ color: '#EF5644' }} onClick={() => resetFilter()}>
...
const AddButton: FC = () => {
    const { openAddModal } = useContext(CTX)
    return (
        <button
...
const AdvancedSearchModal: FC = () => {
    const { filter, changeFilter } = useContext(CTX)
