...
export default class Form extends Component{
    ...
    getValue(input){
        ...
        if(typeof field === 'string'){
          if(field.indexOf('value.') !== -1 || field.indexOf('props.') !== -1 || field.indexOf('input.') !== -1){
            try{
                let evalText = `a = ${field}`
                eval(evalText); 
            } 
            catch{
                a = a;
        ...
        if(a === undefined){return this.getDefault(input)}
        return a
    }
    input_layout(obj){
            ...
            row:[
                {show:!!inlineLabel,html:inlineLabel},
                {
                    ...
                    column:[
                        ...
                        {html:<AIOInput {...input} value={this.getValue(input)}/>},
